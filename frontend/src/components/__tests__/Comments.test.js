...
describe("Comments Component", () => {
  ...
  test("показывает ошибку при попытке отправить пустой комментарий", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText("Комментарий не может быть пустым"),
      ).toBeInTheDocument();
    });
  });
  ...
  test("показывает ошибку если комментарий слишком длинный", async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText(/больше 500 символов/i)).toBeInTheDocument();
    });
  });
  ...
  test("показывает кнопку удаления только для собственных комментариев", () => {
    ...
    // Ищем кнопку удаления только для комментария с id=2
